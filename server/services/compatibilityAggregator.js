...
async function getOrGenerateCompatibility(tenantId, listing, tenantProfile, forceLlm = true) {
  ...
  if (existing && !cache.isStale(existing, listing, tenantProfile)) {
    // If we want LLM but the cached score was only generated by the rule engine,
    // bypass the cache to upgrade the score using Groq.
    const isRuleEngineOnly = existing.generatedBy === "rule-engine";
    const canUpgradeToLlm = forceLlm && isRuleEngineOnly && process.env.GROQ_API_KEY;
